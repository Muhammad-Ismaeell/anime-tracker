...
@extend_schema(
    ...
    responses={201: ErrorSerializer, 400: ErrorSerializer},
)
@api_view(["POST"])
...
@extend_schema(
    ...
    responses={200: LoginResponseSerializer, 400: ErrorSerializer, 403: ErrorSerializer},
)
@api_view(["POST"])
@permission_classes([AllowAny])
def login(request):
...
@extend_schema(
    ...
    responses={200: RefreshResponseSerializer, 401: ErrorSerializer},
)
@api_view(["POST"])
@permission_classes([AllowAny])
...
@extend_schema(
    ...
    responses={200: LoginResponseSerializer, 400: ErrorSerializer},
)
@api_view(["POST"])
@permission_classes([AllowAny])
def google_login(request):
    ...
    if not email or not google_sub:
        return Response({"detail": "Invalid Google identity."}, status=400)
...
def verify_email(request):
    ...
    user, error_message = EmailVerificationService.verify_token(token)
    if error_message:
