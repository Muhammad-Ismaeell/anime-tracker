...
from anime.presentation.serializers import AnimeSerializer
...
class AnimeSearchService:
    ...
    def search(query="", page=1, filters=None):
        ...
        if value := filters.get("type"):
            queryset = queryset.filter(type__iexact=value)

        if value := filters.get("year"):
            queryset = queryset.filter(year=value)

        if value := filters.get("season"):
            queryset = queryset.filter(season__iexact=value)
        ...
        if value := filters.get("rating"):
            queryset = queryset.filter(rating__iexact=value)

        if value := filters.get("genres"):
            queryset = queryset.filter(genres__name__iexact=value).distinct()
        ...
        items = AnimeSerializer(queryset[start:end], many=True).data
